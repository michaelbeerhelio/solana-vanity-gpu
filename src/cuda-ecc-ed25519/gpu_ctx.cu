...
static gpu_ctx_t g_gpu_ctx;
static bool g_gpu_ctx_initialized = false;

bool setup_gpu_ctx() {
    if (g_gpu_ctx_initialized) {
        return true;
    }

    // Initialize mutex
    if (pthread_mutex_init(&g_gpu_ctx.mutex, NULL) != 0) {
        return false;
    }

    // Create CUDA stream
    cudaError_t err = cudaStreamCreate(&g_gpu_ctx.stream);
    if (err != cudaSuccess) {
        pthread_mutex_destroy(&g_gpu_ctx.mutex);
        return false;
    }

    // Initialize verify context to zeros
    memset(&g_gpu_ctx.verify_ctx, 0, sizeof(verify_ctx_t));

    g_gpu_ctx_initialized = true;
    return true;
}
...
void release_gpu_ctx() {
    ...
    // Cleanup CUDA stream
    cudaStreamDestroy(g_gpu_ctx.stream);

    // Cleanup mutex
    pthread_mutex_destroy(&g_gpu_ctx.mutex);

    // Reset initialization flag
    g_gpu_ctx_initialized = false;
}
